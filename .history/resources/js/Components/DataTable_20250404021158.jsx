...
export default function DataTable({
    ...
    const perPage = parseInt(urlParams.get('per_page')) || meta?.per_page || defaultPerPage;
    ...
    const handleSearch = (e) => {
        ...
        setSearchTerm(value);
        // Use setTimeout for debouncing since we want to keep this component self-contained
    ...
    const getUrlWithParams = (url, page) => {
        ...
        // Set the page parameter if provided
        if (page) {
            targetParams.set('page', page);
        }

        // Set per_page parameter
        targetParams.set('per_page', perPage);

        return `${targetUrl.pathname}?${targetParams.toString()}`;
    };
            ...
            {meta && links && (
                <div className="flex justify-between items-center px-4 py-3 bg-white border-t border-gray-200 sm:px-6">
                    <div className="flex flex-1 justify-between sm:hidden">
                        <Link
                            preserveScroll
                            href={links.prev_page_url ? getUrlWithParams(links.prev_page_url) : '#'}
                        ...
                        <Link
                            preserveScroll
                            href={links.next_page_url ? getUrlWithParams(links.next_page_url) : '#'}
                            className={`relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md ${
                                !links.next_page_url
                                    ? 'bg-gray-100 text-gray-400 cursor-not-allowed'
                                    : 'bg-white text-gray-700 hover:bg-gray-50'
                    ...
                    <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                        ...
                        <div>
                            <nav className="inline-flex relative z-0 -space-x-px rounded-md shadow-sm" aria-label="Pagination">
                                ...
                                {meta.last_page && Array.from({ length: meta.last_page }, (_, i) => i + 1).map(page => (
                                    <Link
                                        key={page}
                                        preserveScroll
                                        href={getUrlWithParams(meta.path, page)}
                                        className={`relative inline-flex items-center px-4 py-2 border text-sm font-medium ${page === meta.current_page ? 'z-10 text-indigo-600 bg-indigo-50 border-indigo-500' : 'text-gray-500 bg-white border-gray-300 hover:bg-gray-50'}`}
                                    >
                                        {page}
                                    </Link>
                                ))}
