    ...
    public function handle($data = [])
    {
        //try {
            ...
            if($data['type'] == 'all_clients')
                $data['users_emails'] = Customer::whereIsActive(1)->whereNotNull('email')->pluck('email')->toArray();

            if($data['type'] == 'specific_clients')
                $data['users_emails'] = Customer::whereIsActive(1)->whereIn('id', $data['users'])->whereNotNull('email')->pluck('email')->toArray();

            if($data['type'] == 'cities_clients'){
                $data['users_emails'] = Customer::whereIsActive(1)->whereNotNull('email')
